...
namespace StewartTitle.Argano.APIs.Utils
{
    public class TransactionProcessor
    {
        ...
        // Check if the current party contains transactions with BDO role
        bool isBDO = false;
        ...
        private void FindUser(IOrganizationService service, ITracingService tracingService, InboundTPSParty party)
        {
            ...
            sequenceQuery.Criteria.AddCondition("internalemailaddress", ConditionOperator.Equal, party.primaryEmail);
            EntityCollection sequenceEntities = service.RetrieveMultiple(sequenceQuery);
            if (sequenceEntities.Entities.Count > 0)
            {
        ...
        private void SearchOfficeLocationBranch(IOrganizationService service, ITracingService tracingService, InboundTPSTransaction transaction)
        {
            ...
            if (!String.IsNullOrEmpty(bdoBranchInfo))
            {
                ...
                {
                    tracingService.Trace($"Office Location Branch: {olbEntities[0].Id}");
                    currentOfficeLocationBranch = olbEntities[0];
                    if (isTPS)
                    {
                        EntityReference marketRef = olbEntities[0].GetAttributeValue<EntityReference>("stt_marketid");
                        if (marketRef != null)
                        {
        ...
        private void UpsertAccount(IOrganizationService service, ITracingService tracingService, InboundTPSParty party)
        {
            ...
            else
            {
                ...
                {
                    tracingService.Trace("Account already exists.");
                }
            }
        }
        ...
        private Entity checkIfContactExists(IOrganizationService service, ITracingService tracingService, InboundTPSParty party, out bool contactExists)
        {
            ...
            if (!String.IsNullOrEmpty(party.enterpriseID))
            {
                ...
                contactByEntIDQuery.ColumnSet = new ColumnSet("contactid", "stt_enterpriseid", "statecode", "accountid", "mobilephone", "telephone1", "emailaddress1");
            ...
            if (contactEntities.Entities.Count == 0)
            {
                ...
                {
                    tracingService.Trace("Contact not found with Enterprise ID. Searching contact by contact finance ID...");
                    QueryExpression contactByFinIDQuery = new QueryExpression("contact");
                    contactByFinIDQuery.ColumnSet = new ColumnSet("contactid", "stt_enterpriseid", "statecode", "accountid", "mobilephone", "telephone1", "emailaddress1");
        ...
        private void UpsertContact(IOrganizationService service, ITracingService tracingService, InboundTPSParty party)
        {
            ...
            if (!String.IsNullOrEmpty(party.enterpriseID))
            {
                contactEntity["stt_enterpriseid"] = party.enterpriseID;
            }
            ...
            if (!String.IsNullOrEmpty(party.contactPhoneNumber) && (!contactExists || String.IsNullOrEmpty(contactEntity.GetAttributeValue<string>("mobilephone"))))
            {
                primaryPhoneSet = true;
                contactEntity["mobilephone"] = party.contactPhoneNumber;
            }
            if (!String.IsNullOrEmpty(party.businessPhoneNumber) && (!contactExists || String.IsNullOrEmpty(contactEntity.GetAttributeValue<string>("telephone1"))))
            {
                contactEntity["telephone1"] = party.businessPhoneNumber;
            }
            if (!String.IsNullOrEmpty(party.primaryEmail) && (!contactExists || String.IsNullOrEmpty(contactEntity.GetAttributeValue<string>("emailaddress1"))))
            {
            ...
            try
            {
                ...
                if (!contactExists)
                {
                    tracingService.Trace("Contact doesn't exist. It will be created.");
        ...
        private void UpsertAddresses(IOrganizationService service, ITracingService tracingService, InboundTPSParty party)
        {
            ...
            if (primaryAddressSet && (!String.IsNullOrEmpty(party.contactAddress1) || !String.IsNullOrEmpty(party.contactCity) || !String.IsNullOrEmpty(party.contactCounty) || !String.IsNullOrEmpty(party.contactState) || !String.IsNullOrEmpty(party.contactZip)))
            {
            ...
            if (!String.IsNullOrEmpty(party.businessAddress1) || !String.IsNullOrEmpty(party.businessCity) || !String.IsNullOrEmpty(party.businessCounty) || !String.IsNullOrEmpty(party.businessState) || !String.IsNullOrEmpty(party.businessZip))
            {
        ...
        private void UpsertTransactionRole(IOrganizationService service, ITracingService tracingService, InboundTPSParty party, InboundTPSTransaction transaction)
        {
            ...
            QueryExpression sequenceQuery = new QueryExpression("stt_transactionrole");
            sequenceQuery.ColumnSet = new ColumnSet("stt_transactionroleid", "statecode");
            ...
            if (sequenceEntities.Entities.Count > 0)
            {
                ...
                transactionEntity = sequenceEntities.Entities[0];
                transactionRoleExists = true;
            }
            else
            {
                tracingService.Trace("Transaction role doesn't exist. It will be created.");
                transactionEntity = new Entity("stt_transactionrole");
            }

            // Set fields
            transactionEntity["stt_transactionid"] = new EntityReference("stt_transaction", transactionId);
            if (contactId != null && contactId != Guid.Empty)
            {
                transactionEntity["stt_contactid"] = new EntityReference("contact", contactId);
            }
            ...
            if (roleInFile != null)
            {
                transactionEntity["stt_rolecode"] = roleInFile;
            }
            if (accountId != null && accountId != Guid.Empty)
            {
                transactionEntity["stt_accountid"] = new EntityReference("account", accountId);
            }
            if (owner != null)
            {
                transactionEntity["ownerid"] = owner;
            }
            ...
            if (!transactionRoleExists)
            {
                ...
                {
                    transactionEntity["stt_isdirectingcient"] = transaction.directingPartyFlag == "true";
                }
                transactionEntity["stt_name"] = $"{party.firstName} {party.lastName} - {transaction.roleinFile} - {fileNumber}";
                Guid transactionRoleId = service.Create(transactionEntity);
                tracingService.Trace($"Transaction role created with ID {transactionRoleId}.");
            }
            else
            {
                if (transactionEntity.GetAttributeValue<OptionSetValue>("statecode").Value != 0)
                {
                    transactionEntity["statecode"] = new OptionSetValue(0);
        ...
        private void ProcessTransaction(IOrganizationService service, ITracingService tracingService, InboundTPSParty party, InboundTPSTransaction transaction)
        {
            ...
            {
                currentTransaction["stt_directingclientid"] = new EntityReference("contact", contactId);
            }
            if (!String.IsNullOrEmpty(transaction.propertyAddress1))
            {
                currentTransaction["stt_address1"] = transaction.propertyAddress1;
            }
            if (!String.IsNullOrEmpty(transaction.propertyAddress2))
            {
                currentTransaction["stt_address2"] = transaction.propertyAddress2;
            }
            if (!String.IsNullOrEmpty(transaction.propertyCity))
            {
                currentTransaction["stt_city"] = transaction.propertyCity;
            }
            if (!String.IsNullOrEmpty(transaction.transactionAmount))
            {
                currentTransaction["stt_transactionamount"] = Convert.ToDecimal(transaction.transactionAmount);
            }
            if (!String.IsNullOrEmpty(transaction.propertyCounty))
            {
                ...
                currentTransaction["stt_countyid"] = GetLookupByAbbr(service, transaction.propertyCounty + " County", "stt_county", "stt_countyid", "stt_name");
            }
            if (!String.IsNullOrEmpty(transaction.propertyState))
            {
                currentTransaction["stt_stateid"] = GetLookupByAbbr(service, transaction.propertyState, "stt_state", "stt_stateid", "stt_abbreviation");
            }
            if (!String.IsNullOrEmpty(transaction.propertyZip))
            {
                currentTransaction["stt_zipcodeid"] = GetLookupByAbbr(service, transaction.propertyZip, "stt_zipcode", "stt_zipcodeid", "stt_name");
            }
            if (!String.IsNullOrEmpty(transaction.division))
            {
                currentTransaction["stt_divisionid"] = GetLookupByAbbr(service, transaction.division, "stt_market", "stt_marketid", "stt_name");
            }
            if (!String.IsNullOrEmpty(transaction.transactionType))
            {
                switch (transaction.transactionType.ToLower())
                {
                    case "new sales": currentTransaction["stt_transactiontypecode"] = new OptionSetValue(924510000); break;
                    case "resale": currentTransaction["stt_transactiontypecode"] = new OptionSetValue(924510001); break;
                    case "refi": currentTransaction["stt_transactiontypecode"] = new OptionSetValue(924510002); break;
                    default: break;
                }
            }
            ...
            if (currentOfficeLocationBranch != null)
            {
                ...
                currentTransaction["stt_officelocationbranchid"] = new EntityReference("stt_officelocationbranch", currentOfficeLocationBranch.Id);
                if (currentOfficeLocationBranch.GetAttributeValue<EntityReference>("stt_brandid") != null)
                {
                    currentTransaction["stt_brandid"] = currentOfficeLocationBranch.GetAttributeValue<EntityReference>("stt_brandid");
                }
            }

            if (!String.IsNullOrEmpty(transaction.BDOBranchInfo))
            {
                currentTransaction["stt_titlecompany"] = transaction.BDOBranchInfo;
            }

            if (!String.IsNullOrEmpty(transaction.fileStartDate))
            {
                currentTransaction["stt_filestarton"] = dateFormater(tracingService, transaction.fileStartDate);
            }
            if (!String.IsNullOrEmpty(transaction.estClosingDate))
            {
                currentTransaction["stt_estimatedclosedon"] = dateFormater(tracingService, transaction.estClosingDate);
            }
            if (!String.IsNullOrEmpty(transaction.finalClosingDate))
            {
                currentTransaction["stt_finalcloseon"] = dateFormater(tracingService, transaction.finalClosingDate);
            }

            if (bdoUser != null)
            {
                tracingService.Trace($"System User ID: {bdoUser.GetAttributeValue<Guid>("systemuserid")}");
                currentTransaction["stt_bdoid"] = new EntityReference("systemuser", bdoUser.GetAttributeValue<Guid>("systemuserid"));
            }
            ...
            if (marketEntity != null && isTPS)
            {
                ...
                currentTransaction["stt_marketid"] = new EntityReference("stt_market", marketEntity.Id);
                ...
                if (sequenceClientDetails.Entities.Count > 0)
                {
                    tracingService.Trace("Client detail found");
                    currentTransaction["stt_directingclientdetailsid"] = new EntityReference("stt_clientdetails", sequenceClientDetails[0].Id);
            ...
            {
                currentTransaction["stt_titlefee"] = new Money(Convert.ToDecimal(transaction.titleFee));
            }
            if (!String.IsNullOrEmpty(transaction.escrowFee))
            {
                currentTransaction["stt_escrowfee"] = new Money(Convert.ToDecimal(transaction.escrowFee));
            }
            if (!String.IsNullOrEmpty(transaction.endorsement))
            {
                currentTransaction["stt_endorsementfee"] = new Money(Convert.ToDecimal(transaction.endorsement));
            }
            if (!String.IsNullOrEmpty(transaction.abstractFee))
            {
                currentTransaction["stt_abstractfee"] = new Money(Convert.ToDecimal(transaction.abstractFee));
            }
            if (!String.IsNullOrEmpty(party.partySystem))
            {
                currentTransaction["stt_source"] = party.partySystem;
            }
            if (!String.IsNullOrEmpty(transaction.salesPrice))
            {
                currentTransaction["stt_salesprice"] = new Money(Convert.ToDecimal(transaction.salesPrice));
            }
            if (!String.IsNullOrEmpty(transaction.loanAmount))
            {
                currentTransaction["stt_loanamount"] = new Money(Convert.ToDecimal(transaction.loanAmount));
            }

            currentTransaction["stt_istransaction"] = isTPS;

            currentTransaction["stt_filenumber"] = fileNumber;
            if (!String.IsNullOrEmpty(transaction.fileStatus))
            {
                switch (transaction.fileStatus.ToLower())
                {
                    case "open": currentTransaction["stt_transactionstatuscode"] = new OptionSetValue(924510000); break;
                    case "closed": currentTransaction["stt_transactionstatuscode"] = new OptionSetValue(924510001); break;
                    case "canceled": currentTransaction["stt_transactionstatuscode"] = new OptionSetValue(924510002); break;
                    default: break;
                }
            }
            if (owner != null)
            {
                currentTransaction["ownerid"] = owner;
            }
            ...
            if (!transactionExists)
            {
                try
                {
                    currentTransaction["stt_transactionfinanceidtext"] = transaction.transactionID;
            ...
            else
            {
                ...
                {
                    currentTransaction["statecode"] = new OptionSetValue(0);
                }

                tracingService.Trace("To update.");
                service.Update(currentTransaction);
                tracingService.Trace("Updated.");
            }
            if (bdoUser == null && (contactId != null && contactId != Guid.Empty))
            {
            ...
            try
            {
                ...
                if (party.Transactions != null && party.Transactions.Length > 0)
                {
                    if (!String.IsNullOrEmpty(party.Transactions[0].roleinFile))
                    {
                        ...
                        if (isTPS)
                        {
                            isBDO = party.Transactions[0].roleinFile.ToLower() == "bdo";
                        }
                    }
                }
                if (!ignoreParty)
