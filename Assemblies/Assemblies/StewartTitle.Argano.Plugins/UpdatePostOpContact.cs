            ...
            try
            {
                if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity targetEntity)
                {
                    if (targetEntity.LogicalName == "contact")
                    {
                        ...
                        ColumnSet existingColumnSet = new ColumnSet("stt_enterpriseid", "stt_contactfinanceidtext", "stt_countyid", "firstname", "lastname", "emailaddress1", "telephone1", "address1_line1", "address1_city", "address1_stateorprovince", "address1_postalcode", "parentcustomerid", "address1_line2", "jobtitle");
                        ...
                        if (contactToUpdate.ContainsData("parentcustomerid"))
                        {
                            ...
                            ColumnSet accountColumnSet = new ColumnSet("name", "address1_line1", "address1_city", "address1_stateorprovince", "address1_country", "address1_postalcode");
                        ...
                        if (contactToUpdate != null)
                        {
                            ...
                            if (accountToUpdate != null)
                            {
                                ...
                                if (accountToUpdate.Contains("address1_stateorprovince"))
                                {
                                    QueryExpression stateQuery = new QueryExpression("stt_state");
                                    stateQuery.ColumnSet = new ColumnSet("stt_abbreviation");
                                    stateQuery.Criteria.AddCondition("stt_name", ConditionOperator.Equal, accountToUpdate.GetAttributeValue<string>("address1_stateorprovince"));
                                    ...
                                    {
                                        accountPayload.address1_stateorprovince = sequenceEntities[0].GetAttributeValue<string>("stt_abbreviation");
                                    }
                                }
                                ...
                                if (accountToUpdate.Contains("address1_postalcode"))
                                {
                                    accountPayload.address1_postalcode = accountToUpdate.GetAttributeValue<string>("address1_postalcode");
                                }
                            }
                            ...
                            tracingService.Trace("Setting state.");
                            if (contactToUpdate.ContainsData("address1_stateorprovince"))
                            {
                                QueryExpression stateQuery = new QueryExpression("stt_state");
                                stateQuery.ColumnSet = new ColumnSet("stt_abbreviation");
                                stateQuery.Criteria.AddCondition("stt_name", ConditionOperator.Equal, contactToUpdate.GetAttributeValue<string>("address1_stateorprovince"));
                                EntityCollection sequenceEntities = service.RetrieveMultiple(stateQuery);
                                if (sequenceEntities != null)
                                {
                                    contactPayload.address1_stateorprovince = sequenceEntities[0].GetAttributeValue<string>("stt_abbreviation");
                                }
                            }

                            tracingService.Trace("Setting zip.");
                            if (contactToUpdate.ContainsData("address1_postalcode"))
                            {
                                contactPayload.address1_postalcode = contactToUpdate.GetAttributeValue<string>("address1_postalcode");
                            }
                        ...
                        {
                            tracingService.Trace("Contact not found, plugin will exit.");
                            return;
                        }
                    }
                    else
